...
if nVP==0:
    ...
    nonno = 1
    if nonno==1:
        nudam = Function(DG0).interpolate(conditional( inner(grad(etamp),grad(etamp)) < betawb**2, 0.0, nub0 ))
    else:
        nudam.interpolate( conditional( lt(inner(grad(etamp),grad(etamp)), betawb**2), 0.0, nub0 ) )
        nudam.interpolate( conditional( (inner(grad(etamp),grad(etamp)), betawb**2), 0.0, nub0 ) )
    ...
    phiexprnl1 = replace(phiexprnl1, {phi1: 2.0*phimp-phi0})
    phiexprnl1 = replace(phiexprnl1, {eta1: 2.0*etamp-eta0})
    # etaweakfwavebr = Function(DG0).interpolate(conditional( inner(grad(eta0),grad(eta0)) < (betawb+0.0*x[0]*x[1])**2, 0.0*nub0 + 0.0*x[0]*x[1], nub0 + 0.0*x[0]*x[1] )) * (inner(grad(vvmphi),grad(etamp))) * dx
    etaweakfwavebr = nudam * (inner(grad(vvmphi),grad(etamp))) * dx
    #etaweakfwavebr = ( conditional( inner(grad(eta0),grad(eta0)) < (betawb**2+0.0*x[0]*x[1])**2, 0.0*nub0 + 0.0*x[0]*x[1], nub0 + 0.0*x[0]*x[1] ) * inner(grad(vvmphi),grad(etamp)) ) * dx
    
    etaexprnl1 = derivative(VPbl, etamp, du=vvmpeta) # evolution eqn for phi! ( (phi1-phi0)/dt + ... + nubdamp * ( inner(grad(vvmpeta),grad(phimp)) ) ) * dx
    etaexprnl1 = replace(etaexprnl1, {phi1: 2.0*phimp-phi0})
    etaexprnl1 = replace(etaexprnl1, {eta1: 2.0*etamp-eta0})
    # phiweakfwavebr = Function(DG0).interpolate(conditional( inner(grad(eta0),grad(eta0)) < (betawb+0.0*x[0]*x[1])**2, 0.0*nub0 + 0.0*x[0]*x[1], nub0 + 0.0*x[0]*x[1] )) * ( inner(grad(vvmpeta),grad(phimp)) ) * dx
    phiweakfwavebr = nudam * ( inner(grad(vvmpeta),grad(phimp)) ) * dx
    #phiweakfwavebr = ( conditional( inner(grad(eta0),grad(eta0)) < (betawb**2+0.0*x[0]*x[1])**2, 0.0*nub0 + 0.0*x[0]*x[1], nub0 + 0.0*x[0]*x[1] ) * inner(grad(vvmpeta),grad(phimp)) ) * dx    

    # Combine the 2 weak formas and solve as coupled system using mix variables function space
...
while t <= T:
      ...
      if nVP==0:
          ...
          eta0.interpolate(2.0*etamp-eta0)
          # ISSUE:
          # nubdamp = conditional( inner(grad(eta0),grad(eta0)) < (betawb+0.0*x[0]*x[1])**2, 0.0*nub0 + 0.0*x[0], nub0 + 0.0*x[0] )
          # nubdamp = nub0 * 0.5 * (1+tanh(kk*(inner(grad(eta0),grad(eta0))-(betawb+0.0*x[0]*x[1])**2) ) )
          # nudam =  Function(DG0).interpolate(conditional( inner(grad(eta0),grad(eta0)) < (betawb+0.0*x[0]*x[1])**2, 0.0*nub0 + 0.0*x[0], nub0 + 0.0*x[0] ))
          # nudam.interpolate(conditional( inner(grad(eta0),grad(eta0)) < (betawb+0.0*x[0]*x[1])**2, 0.0*nub0 + 0.0*x[0]*x[1], nub0 + 0.0*x[0]*x[1] ))
          # nubdamp = Function(DG0).interpolate(conditional( inner(grad(eta0),grad(eta0)) < (betawb+0.0*x[0]*x[1])**2, nub0 + 0.0*x[0], nub0 + 0.0*x[0] ))
          # Works:
          # nudam = Function(DG0).interpolate(conditional( inner(grad(eta0),grad(eta0)) < (betawb+0.0*x[0]*x[1])**2, 0.0*nub0 + 0.0*x[0]*x[1], nub0 + 0.0*x[0]*x[1] ))
          #
          if nonno==0:
              nudam = Function(DG0).interpolate(conditional( inner(grad(etamp),grad(etamp)) < betawb**2, 0.0, nub0))
          else:
              nudam.interpolate( conditional( lt(inner(grad(etamp),grad(etamp)), betawb**2), 0.0, nub0 ))
              # Not sure whqt lt does: nudam.interpolate( conditional( (inner(grad(etamp),grad(etamp)), betawb**2), 0.0, nub0 )) 
          #  nudam = Function(DG0).interpolate(conditional( (eta1 - eta0)/dt < alphas*sqrt(1+epsilon)*etamp, 0.0*nub0, nub0 ))
          #  nudam = replace(nudam, {eta1: 2.0*etamp-eta0})
          # nudamm = Function(DG0).interpolate(conditional( inner(grad(eta0),grad(eta0)) < (betawb+0.0*x[0]*x[1])**2, 0.0*nub0 + 0.0*x[0]*x[1], nub0 + 0.0*x[0]*x[1] ))
          # nudamm = Function(DG0).interpolate(conditional( inner(grad(eta0),grad(eta0)) < (betawb+0.0*x[0]*x[1])**2, 0.0*nub0 + 0.0*x[0]*x[1], nub0 + 0.0*x[0]*x[1] ))
          nudamm.interpolate(nudam)
          # nudamm.interpolate(nudam)
          #  nudam.interpolate(nudam) # Put into display variable.
          if t>tmE+smallfac:
